...
// --------- Interrupt Timer for Ultrasonic ---------
static volatile _Atomic bool tick_obstacle = false;
static volatile _Atomic bool obst_blocked  = false;  // stays true if decision says STOP


static bool cb_obstacle(repeating_timer_t *rt) {
    tick_obstacle = true;     // ISR context — just set flag
    return true;              // keep repeating
}

// static helpers for printing
...
int main(void) {
    ...
    // ---- Repeating timer for ultrasonic obstacle detection (runs every 60 ms) ----
    repeating_timer_t t_obst;
    add_repeating_timer_ms(+60, cb_obstacle, NULL, &t_obst);
    ...
    while (true) {
        ...
        // ---- Read dedicated line-follow sensor D0 (GP13) with short hold/debounce ----
         // Pause LF entirely while blocked
    if (obst_blocked) {
        motor_stop();
        left_cmd_pct  = 0.0f;   // reset slew accumulators
        right_cmd_pct = 0.0f;
    } else {
        // ---- Read dedicated line-follow sensor D0 (GP13) with short hold/debounce ----
        ...
        float err_raw = lf_d0_now ? +1.0f : -1.0f;
    #if LF_INVERT_STEER
        err_raw = -err_raw;
    #endif
        ...
        if (lf_cmd_sign == 0) { lf_cmd_sign = want_sign; lf_flip_ms = 0; }
        else if (want_sign != lf_cmd_sign) {
            lf_flip_ms += SAMPLE_IR_MS;
            if (lf_flip_ms >= LF_FLIP_CONFIRM_MS) { lf_cmd_sign = want_sign; lf_flip_ms = 0; }
        } else { lf_flip_ms = 0; }

        float mag = fabsf(err_raw);
        float err = (float)lf_cmd_sign * mag;

        static float prev_err_f = 0.0f;
        float derr = err - prev_err_f; prev_err_f = err;

        float u = LF_KP * err + LF_KD * derr;
        ...
        if (fabsf(u_filt) > 0.6f) { base -= 4.0f; turn_gain *= 1.10f; }
    ...
    #if DRIFT_PID_ENABLE
        {
            ...
            float fade = 1.0f - fminf(1.0f, fabsf(u_filt));
            drift_trim_pct *= fade;
        }
    #endif
    ...
    motor_set_speed(0.35f, 0.35f);
        ...
        // Ultrasonic tick and detection --------------------------------
if (tick_obstacle) {
    tick_obstacle = false;

    // Run the 60 ms monitor tick
    det_state_t st = obst_monitor_tick(&mon);

    int hits=-1, valids=-1;
    obst_get_last_gate_stats(&hits, &valids);
    if (mon.valid) {
        printf("CENTER: %.0f mm | gate: hits=%d/%d -> breached=%s\n",
               mon.distance_mm, hits, valids, (st==DET_BREACHED?"YES":"NO"));
    } else {
        printf("CENTER: timeout | gate: hits=%d/%d -> breached=%s\n",
               hits, valids, (st==DET_BREACHED?"YES":"NO"));
    }

    // --- Auto-clear logic: require a few consecutive "clear" samples ---
    static int clear_ctr = 0;              // counts consecutive NOT-breached samples
    if (st == DET_BREACHED) {
        clear_ctr = 0;                     // reset counter while breached
    } else {
        if (clear_ctr < 3) clear_ctr++;    // 3*60ms ≈ 180 ms of clear
        if (clear_ctr >= 3) obst_blocked = false;
    }

    // --- Edge-trigger the sweep ONLY on transition to breached ---
    if (st == DET_BREACHED && !obst_blocked) {
        obst_blocked = true;               // gate LF
        motor_stop();

        puts("→ Sweep triggered...");
        decision_t dec = obst_sweep_and_decide();

        float Ldeg=0,Lmm=-1,Rdeg=0,Rmm=-1,Wmm=-1;
        bool have_obs = obst_get_last_obstacle_summary(&Ldeg,&Lmm,&Rdeg,&Rmm,&Wmm);
        if (have_obs) {
            printf("EDGES: L@%.1f°(%.0f mm), R@%.1f°(%.0f mm)\n", Ldeg,Lmm,Rdeg,Rmm);
            if (Wmm > 0) printf("WIDTH: ≈ %.0f mm\n", Wmm);
            else puts("WIDTH: unavailable (insufficient geometry)");
        } else {
            puts("EDGES/WIDTH: no blocking obstacle detected in sweep");
        }

        switch (dec.action) {
            case ACT_GO:
                puts("DECISION: GO");
                obst_blocked = false;      // allow LF to resume immediately
                break;
            case ACT_TURN_LEFT:
                printf("DECISION: TURN LEFT by %.0f°\n", dec.turn_deg);
                // If you actually execute a turn here, do it, then:
                obst_blocked = false;      // re-enable LF after command
                break;
            case ACT_TURN_RIGHT:
                printf("DECISION: TURN RIGHT by %.0f°\n", dec.turn_deg);
                // If you actually execute a turn here, do it, then:
                obst_blocked = false;
                break;
            case ACT_STOP:
            default:
                puts("DECISION: STOP (blocked across sweep)");
                // remain blocked; auto-clear will release when the gate is clear
                break;
        }
        puts("");
    }
